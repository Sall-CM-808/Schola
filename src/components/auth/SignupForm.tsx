...
const SignupForm: React.FC = () => {
  ...
  const roles = [
    { value: "student", label: "Étudiant", icon: User },
    { value: "teacher", label: "Enseignant", icon: GraduationCap },
    { value: "admin", label: "Administrateur", icon: User },
  ];
      ...
      >
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-white/70">Créez votre compte Schola</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="relative">
              ...
              <div className="relative">
                <GraduationCap className="absolute left-3 top-1/2 transform -translate-y-1/2 text-white/50 w-5 h-5" />
                <select
                  name="role"
                  value={formData.role}
                  onChange={handleChange}
                  className="w-full pl-12 pr-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-[#b8d070] focus:border-transparent transition-all duration-300 appearance-none"
                >
                  {roles.map((role) => (
                    <option
                      key={role.value}
